# Object listing scripts
# 
# Author: paul
###############################################################################


#' .ls.objects
#' 
#' List Objects
#' #' 
#' Thanks to http://stackoverflow.com/questions/1358003/tricks-to-manage-the-available-memory-in-an-r-session
#' 
#' @param pos 
#' @param pattern 
#' @param order.by 
#' @param decreasing 
#' @param head 
#' @param n Number of Objects to List
#' @return A list of objects in the current workspace
#' @author gb02413
#' @export
.ls.objects <- function (pos = 1, pattern, order.by,
		decreasing=FALSE, head=FALSE, n=5) {
	napply <- function(names, fn) sapply(names, function(x)
					fn(get(x, pos = pos)))
	names <- ls(pos = pos, pattern = pattern)
	obj.class <- napply(names, function(x) as.character(class(x))[1])
	obj.mode <- napply(names, mode)
	obj.type <- ifelse(is.na(obj.class), obj.mode, obj.class)
	obj.prettysize <- napply(names, function(x) {
				capture.output(print(object.size(x), units = "auto")) })
	obj.size <- napply(names, object.size)
	obj.dim <- t(napply(names, function(x)
						as.numeric(dim(x))[1:2]))
	vec <- is.na(obj.dim)[, 1] & (obj.type != "function")
	obj.dim[vec, 1] <- napply(names, length)[vec]
	out <- data.frame(obj.type, obj.size, obj.prettysize, obj.dim)
	names(out) <- c("Type", "Size", "PrettySize", "Rows", "Columns")
	if (!missing(order.by))
		out <- out[order(out[[order.by]], decreasing=decreasing), ]
	if (head)
		out <- head(out, n)
	out
}


#' lsos
#' 
#' List R Objects by Size
#' 
#' Thanks to http://stackoverflow.com/questions/1358003/tricks-to-manage-the-available-memory-in-an-r-session
#' @param ... 
#' @param n Number of Objects to List
#' @return A datafrmae listing objects
#' @author gb02413
#' @export
lsos <- function(..., n=10) {
	.ls.objects(..., order.by="Size", decreasing=TRUE, head=TRUE, n=n)
}


#' proc.time.to.data.frame
#' 
#' Convert a \code{proc.time} result to a dataframe
#' @param x 
#' @param name 
#' @return a dataframe
#' 
#' @author paul
#' @export
proc.time.to.data.frame<-function(x, name=NA){
	if(length(x)==5){
		result<-data.frame(name=name, 
				user=as.numeric(x['user.self']),
				sys=as.numeric(x['sys.self']),
				elapsed=as.numeric(x['elapsed']))
	}
	if(length(x)>5){
		warning("Doesn't work with vecotrs of times yet !")
	}
	return(result)
}

